 ...
 * The DArray allows the client to store any number of elements of any
 * desired base type and is appropriate for a wide variety of storage
 * problems.  It supports efficient element access, and
 * appending/inserting/deleting elements as well as optional sorting and
 * searching.  In all cases, the DArray imposes no upper bound on the
 * number of elements and deals with all its own memory management.  The
 * client specifies the size (in bytes) of the elements that will be
 * stored in the array when it is created. Thereafter the client and the
 * DArray can refer to elements via (void*) ptrs. The memory management 
 * alone more than justifies the cost of the ADT- it should provide you
 * with years of faithful service.  Just wipe occasionally with a damp
 * cloth.
 * ------------------------------------------------------------
 */
 ...
 * DArray, but these variables must be initialized with the result of 
 * ArrayNew.  The DArray is implemented with pointers, so all client copies
 * in variables or parameters will be "shallow" -- they will all actually
 * pounsigned long to the same DArray structure.  Only calls to ArrayNew
 * create new arrays.  The struct declaration below is "incomplete"- the
 ...
 * (const void*) pointers (these will pounsigned long to elements) and
 * returns an int.  The comparator should indicate the ordering of the
 * two elements using the same convention as the strcmp library function:
 ...
 * ArrayNew
 * --------
 * Creates a new DArray and returns it.  There is zero element in the
 * array to start.  The elemSize parameter specifies the number of
 * bytes that a single element of this array should take up.  For example,
 * if you want to store elements of type Binky, you would pass
 * sizeof (Binky) as this parameter.  An assert is raised if the size is
 * not greater than zero.
 *
 * The numElemsToAllocate parameter specifies the initial allocated length 
 * of the array, as well as the dynamic reallocation increment for when
 * the array grows.  Rather than growing the array one element at a time
 * as elements are added (which is rather inefficient), you will grow the
 * array in chunks of numElemsToAllocate size.  The "allocated length" is
 * the number of elements that have been allocated, the "logical length"
 * is the number of those slots actually being currently used.
 * 
 * A new array is initially allocated to the size of numElemsToAllocate,
 * the logical length is zero.  As elements are added, those allocated
 * slots fill up and when the initial allocation is all used, grow the
 * array by another numElemsToAllocate elements. You will continue
 * growing the array in chunks like this as needed.  Thus the allocated
 * length will always be a multiple of numElemsToAllocate.  Don't worry
 * about using realloc to shrink the array allocation if many elements
 * are deleted from the array.  It turns out that many implementations
 * of realloc don't even pay attention to such a request so there is
 * little pounsigned long in asking.  Just leave the array over-allocated.
 *
 ...
 * array when the entire array is being freed (using ArrayFree).  This
 * function is your chance to do any deallocation/cleanup required for the
 * element (such as freeing any pointers contained in the element). The
 * client can pass NULL for the cleanupFn if the elements don't require
 * any handling on free. 
 */
 ...
 * Numbering begins with 0.  An assert is raised if n is less than 0 or 
 * greater than the logical length minus 1. Note this function returns a 
 * pointer into the DArray's element storage, so the pointer should be
 * used with care.  This function must operate in constant time.
 *
...
void ArrayInsertAt (DArray _arrayObject, const void *element, unsigned long n);
...
void ArrayReplaceAt (DArray _arrayObject, const void *element, unsigned long n);
